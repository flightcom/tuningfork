...
<table class="table table-hover tablesorter table-list-instru col-xs-12">
	...
	<thead>
		...
		<tr>
			...
			<th class="col-xs-2 filter-select filter-onlyAvail" data-placeholder="Sélectionner">Marque</th>
			<th class="col-xs-2">Modèle</th>
			<th class="col-xs-2" data-col="serial">Numéro de série</th>
			<th class="col-xs-2" data-col="date">Date entrée</th>
		</tr>
	...
	<tbody>
		...
		<!-- <tr ng-repeat="instrument in filteredInstruments" ng-click="go('/admin/instruments/' + instrument.instru_id)" style="cursor:pointer;" class="border-state border-state-<?php echo $i->instru_etat; ?>" ng-class="instrument.instru_dispo =='1' ? 'border-left-ok' : 'border-left-ko'"> -->
		<tr onclick="location.href='/admin/instruments/<?php echo $i->instru_id; ?>'" style="cursor:pointer;" class="border-state border-state-<?php echo $i->instru_etat; ?> <?php echo $i->instru_dispo =='1' ? 'border-left-ok' : 'border-left-ko'; ?>">
		    <td class="col-xs-1"><?php echo $i->instru_id; ?></td>
		    <td class="col-xs-2"><?php echo $i->categ_pathname; ?></td>
		    <td class="col-xs-2"><?php echo $i->marque_nom; ?></td>
		    <td class="col-xs-2 hidden-xs hidden-sm"><?php echo $i->instru_modele; ?></td>
		    <td class="col-xs-2" data-col="serial"><?php echo $i->instru_numero_serie; ?></td>
		    <td class="col-xs-2" data-col="date"><?php echo $i->instru_date_entree; ?></td>
